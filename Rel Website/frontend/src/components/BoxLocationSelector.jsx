...
/**
 * BoxLocationSelector - A dropdown with custom entry capability
 * Allows selecting from predefined values or entering custom values
 */
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <div
        ...
        className={`relative w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 cursor-pointer transition-colors ${
      ...
      >
        ...
        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 pointer-events-none">
          {value && !disabled && (
            <button
              onClick={handleClear}
              className="pointer-events-auto p-1 hover:bg-slate-100 dark:hover:bg-slate-600 rounded"
            >
