...
import {
  ...
  hasRetentionData,
  BOX_LOCATION_DEFAULTS,
  SAMPLE_CARRIERS,
} from '../constants/retentionConstants';

/**
 * RetentionSubBox - Individual retention type sub-box
 */
function RetentionSubBox({
  title,
  note,
  data,
  onChange,
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-600 rounded-lg p-4 bg-slate-50 dark:bg-slate-800/50">
      <div className="mb-3">
        <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">
          {title}
        </h4>
          ...
          <div key={field.key} className={field.span ? `sm:col-span-${field.span}` : ''}>
            ...
            {field.type === 'date' ? (
              ...
              <BoxLocationSelector
                ...
                onAddCustom={onAddBoxLocation}
                placeholder={field.placeholder || 'Select or enter…'}
                disabled={disabled}
              />
            ) : field.type === 'number' ? (
              <input
...
/**
 * EnhancedRetentionDetails - Main component with three sub-boxes
 */
export default function EnhancedRetentionDetails({
  request,
  isEditing,
  setIsEditing,
  onSave,
  ...
  const [retentionData, setRetentionData] = useState(() =>
    parseRetentionDetails(request.retention_details)
  );
  ...
  const handleAddBoxLocation = (newLocation) => {
    setRetentionData((prev) => ({
      ...prev,
      boxLocations: [...new Set([...prev.boxLocations, newLocation])],
    }));
  };
  ...
  return (
    <div className="space-y-4">
      {/* Three retention sub-boxes */}
      <RetentionSubBox
        title="A. Reliability Tested Units"
        data={retentionData.retentionData.reliabilityTested}
        onChange={(data) =>
          setRetentionData((prev) => ({
            ...prev,
            retentionData: { ...prev.retentionData, reliabilityTested: data },
          }))
        }
        boxLocations={retentionData.boxLocations}
        onAddBoxLocation={handleAddBoxLocation}
        fields={reliabilityFields}
        disabled={disabled}
      />

      <RetentionSubBox
        title="B. Excess Units"
        note="N/A if no excess"
        data={retentionData.retentionData.excessUnits}
        onChange={(data) =>
          setRetentionData((prev) => ({
            ...prev,
            retentionData: { ...prev.retentionData, excessUnits: data },
          }))
        }
        boxLocations={retentionData.boxLocations}
        onAddBoxLocation={handleAddBoxLocation}
      ...
      <RetentionSubBox
        title="C. Sent to Tanyag Units"
        data={retentionData.retentionData.sentToTanyag}
        onChange={(data) =>
          setRetentionData((prev) => ({
            ...prev,
            retentionData: { ...prev.retentionData, sentToTanyag: data },
          }))
        }
        boxLocations={retentionData.boxLocations}
        onAddBoxLocation={handleAddBoxLocation}
