...
const roleBadgeColors = {
  ...
  'REL Engineer':  'bg-blue-100 text-blue-700 border-blue-200',
...
function AddUserModal({ open, onClose, onCreated }) {
  const [form, setForm] = useState({ email: '', username: '', password: '', role: 'Analyst' });
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          <div>
            ...
            <input type="email" required value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
              className="w-full border border-slate-200 rounded-lg px-3 py-2.5 bg-slate-50 focus:bg-white focus:border-violet-500 focus:ring-2 focus:ring-violet-200 text-sm" />
          </div>
          <div>
...
const PERMISSION_LABELS = {
  ...
  edit_request:    'Edit CA Information',
...
const CONFIGURABLE_ROLES = ['REL Engineer', 'Analyst', 'Technician', 'Planner'];
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
        <div>
          <h3 className="font-heading font-semibold text-slate-800 dark:text-white">Role Permissions</h3>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
              <th className="text-left px-6 py-3 text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400 min-w-[180px]">Permission</th>
              {CONFIGURABLE_ROLES.map(role => (
                <th key={role} className="text-center px-4 py-3 text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400 min-w-[120px]">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
            {allPerms.map(perm => (
              <tr key={perm} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  ...
                  <td key={role} className="px-4 py-3 text-center">
                    <button
                      ...
                      className={`w-9 h-5 rounded-full transition-colors relative ${
                        permissions[role]?.[perm] ? 'bg-emerald-500' : 'bg-slate-200'
                      }`}
                    >
...
export default function Users() {
  ...
  const canManageUsers = isAdmin || hasPerm('manage_users');
  ...
  const handleDelete = async (userId) => {
    ...
    const status = target?.user_status || (target?.blocked ? 'lock' : target?.approved ? 'approved' : 'pending');
    if (status === 'approved') {
      alert('Cannot delete an approved user. Change their status to Hold, Lock, or Declined first.');
      return;
    }
  ...
  const handleApprove  = async (userId) => {
    try { await api.approveUser(userId); loadUsers(false); } catch (err) { alert(err.response?.data?.detail || err.message); }
  };
  const handleReject   = async (userId) => {
    try { await api.rejectUser(userId); loadUsers(false); } catch (err) { alert(err.response?.data?.detail || err.message); }
  };

  const handleEditRole = (userId, currentRole) => { setEditingRole(userId); setTempRole(currentRole); };
  const handleRoleChange = async (userId) => {
    try { await api.updateUserRole(userId, tempRole); setEditingRole(null); loadUsers(false); }
    catch (err) { alert(err.response?.data?.detail || err.message); setEditingRole(null); }
  };
  const handleCancelRoleEdit = () => { setEditingRole(null); setTempRole(''); };

  const handleEditStatus = (userId, currentStatus) => { setEditingStatus(userId); setTempStatus(currentStatus || 'pending'); };
  const handleStatusChange = async (userId) => {
    try { await api.updateUserStatus(userId, tempStatus); setEditingStatus(null); loadUsers(false); }
    catch (err) { alert(err.response?.data?.detail || err.message); setEditingStatus(null); }
  };
  const handleCancelStatusEdit = () => { setEditingStatus(null); setTempStatus(''); };

  const handleEditUsername = (userId, current) => { setEditingUsername(userId); setTempUsername(current); };
  const handleUsernameChange = async (userId) => {
    if (!tempUsername.trim()) return;
    try { await api.updateUserUsername(userId, tempUsername.trim()); setEditingUsername(null); loadUsers(false); }
    catch (err) { alert(err.response?.data?.detail || err.message); setEditingUsername(null); }
  };
  const handleCancelUsernameEdit = () => { setEditingUsername(null); setTempUsername(''); };

  const handleToggleBlock = async () => {
    if (!blockConfirm) return;
    try { await api.toggleBlockUser(blockConfirm.id); loadUsers(false); }
    catch (err) { alert(err.response?.data?.detail || err.message); }
  ...
  return (
    <div className="space-y-6 stagger-children">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          {canManageUsers && (
            <button onClick={() => setShowAdd(true)}
              className="inline-flex items-center gap-2 bg-slate-900 hover:bg-slate-800 dark:bg-slate-700 dark:hover:bg-slate-600 text-white rounded-lg px-3.5 py-2 font-medium text-sm shadow-sm transition-all">
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                ...
                <th className="text-left px-6 py-3 text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400">Online</th>
                <th className="text-left px-6 py-3 text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400">Created</th>
                <th className="text-right px-6 py-3 text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {users.map(u => (
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <div
                          className={`w-8 h-8 rounded-full overflow-hidden flex items-center justify-center text-xs font-bold uppercase ${u.blocked ? 'bg-red-100 text-red-500' : 'bg-slate-200 text-slate-600'} ${u.avatar ? 'cursor-pointer ring-1 ring-transparent hover:ring-violet-400 transition-all' : ''}`}
                      ...
                      {editingUsername === u.id ? (
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            className="border border-slate-300 rounded-lg px-2 py-1 text-xs w-28 focus:border-violet-500 focus:ring-2 focus:ring-violet-200"
                            autoFocus
                          />
                    ...
                    {editingRole === u.id ? (
                      <div className="flex items-center gap-2">
                        <select value={tempRole} onChange={e => setTempRole(e.target.value)}
                          className="border border-slate-300 rounded-lg px-2 py-1 text-xs focus:border-violet-500 focus:ring-2 focus:ring-violet-200" autoFocus>
                          {ROLES.map(r => <option key={r} value={r}>{r}</option>)}
                        </select>
                        <button onClick={() => handleRoleChange(u.id)} className="p-1 rounded hover:bg-emerald-50 text-emerald-600" title="Save"><Check className="w-4 h-4" /></button>
                        <button onClick={handleCancelRoleEdit} className="p-1 rounded hover:bg-slate-100 text-slate-400" title="Cancel"><X className="w-4 h-4" /></button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">
                        ...
                        {isAdmin && (
                          <button onClick={() => handleEditRole(u.id, u.role)} className="p-1 rounded hover:bg-slate-100 text-slate-400 hover:text-slate-600" title="Edit role">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-400">
                    {u.created_at ? (
                      <>
                        ...
                        <div className="text-xs">{new Date(u.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                      </>
                    ) : '—'}
                  </td>
                    ...
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {isAdmin && u.id !== currentUser?.id && (
                        <button
                          ...
                          className={`p-1.5 rounded transition-colors ${u.blocked ? 'hover:bg-emerald-50 text-red-400 hover:text-emerald-600' : 'hover:bg-red-50 text-slate-300 hover:text-red-500'}`}>
                          <Ban className="w-4 h-4" />
                        </button>
                      )}
                      {isAdmin && u.id !== currentUser?.id && getUserStatusBadge(u).value !== 'approved' && (
      ...
      {/* Avatar lightbox */}
      {viewingAvatar && (
        <div className="fixed inset-0 z-[9999] bg-black/75 flex flex-col items-center justify-center gap-4"
          onClick={() => setViewingAvatar(null)}>
          <img src={viewingAvatar.src} alt={viewingAvatar.name}
            className="w-52 h-52 rounded-full object-cover shadow-2xl ring-4 ring-white/20"
            onClick={e => e.stopPropagation()} />
          {viewingAvatar.name && <p className="text-white text-sm font-semibold tracking-wide">{viewingAvatar.name}</p>}
          <p className="text-white/50 text-xs">Click anywhere to close</p>
          <button onClick={() => setViewingAvatar(null)}
